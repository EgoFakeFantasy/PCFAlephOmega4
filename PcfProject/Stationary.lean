...
theorem exists_stationary_of_subset_iUnion_of_small
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    {ι : Type v}
    ...
    {T : ι -> Set alpha}
    (hS : Stationary S)
    (hCover : S <= Set.iUnion T) :
    exists i, Stationary (T i) :=
  exists_stationary_of_stationary_iUnion_of_small hcof hι
    (stationary_mono hCover hS)

theorem subset_iUnion_stationaryFiber_of_value_cover
    {alpha : Type u}
    {idx : Type v}
    {S : Set alpha}
    {f : alpha -> alpha}
    {value : idx -> alpha}
    (hCover : forall x, S x -> exists i, f x = value i) :
    S <= Set.iUnion (fun i => StationaryFiber S f (value i)) := by
  intro x hxS
  cases hCover x hxS with
  | intro i hxi =>
      exact Set.mem_iUnion.mpr (Exists.intro i (And.intro hxS hxi))

theorem exists_stationaryFiber_of_small_value_cover
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    {idx : Type v}
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
    (hidx :
      Cardinal.lift.{u, v} (Cardinal.mk idx) <
        Cardinal.lift.{v, u} (Order.cof alpha))
    {S : Set alpha}
    {f : alpha -> alpha}
    {value : idx -> alpha}
    (hS : Stationary S)
    (hCover : forall x, S x -> exists i, f x = value i) :
    exists i, Stationary (StationaryFiber S f (value i)) :=
  exists_stationary_of_subset_iUnion_of_small
    (S := S)
    (T := fun i => StationaryFiber S f (value i))
    hcof
    hidx
    hS
    (subset_iUnion_stationaryFiber_of_value_cover hCover)

theorem exists_stationary_subset_eqOn_const_of_small_value_cover
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    {idx : Type v}
    ...
    exists beta, exists T : Set alpha,
      And (T <= S) (And (Stationary T) (Set.EqOn f (fun _ => beta) T)) := by
  cases exists_stationaryFiber_of_small_value_cover hcof hidx hS hCover with
  | intro i hFiber =>
      exact Exists.intro (value i)
        (exists_stationary_subset_eqOn_const_of_stationaryFiber hFiber)

theorem exists_stationary_subset_eqOn_value_of_small_value_cover
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    {idx : Type v}
    ...
    (hS : Stationary S)
    (hCover : forall x, S x -> exists i, f x = value i) :
    exists i, exists T : Set alpha,
      And (T <= S)
        (And (Stationary T) (Set.EqOn f (fun _ => value i) T)) := by
  cases exists_stationaryFiber_of_small_value_cover hcof hidx hS hCover with
  | intro i hFiber =>
      exact Exists.intro i
        (exists_stationary_subset_eqOn_const_of_stationaryFiber hFiber)

theorem exists_stationaryFiber_mem_of_range_subset_small
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
    ...
    {f : alpha -> alpha}
    (hBsmall :
      Cardinal.lift.{u, u} (Cardinal.mk B) <
        Cardinal.lift.{u, u} (Order.cof alpha))
    (hS : Stationary S)
    (hSubset : forall x, S x -> B (f x)) :
    exists beta, And (B beta) (Stationary (StationaryFiber S f beta)) := by
  let idx := B
  let value : idx -> alpha := fun beta => beta.1
  have hCover : forall x, S x -> exists i, f x = value i := by
    intro x hxS
    exact Exists.intro (Subtype.mk (f x) (hSubset x hxS)) rfl
  cases exists_stationaryFiber_of_small_value_cover
      (idx := idx)
      hcof
      (by simpa [idx] using hBsmall)
      hS
      hCover with
  | intro i hi =>
      exact Exists.intro (value i) (And.intro i.2 hi)

theorem exists_stationaryFiber_of_range_subset_small
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
  ...
  cases exists_stationaryFiber_mem_of_range_subset_small
      hcof hBsmall hS hSubset with
  | intro beta hbeta =>
      exact Exists.intro beta hbeta.right

theorem exists_stationary_subset_eqOn_const_of_range_subset_small
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
    ...
    exists beta, exists T : Set alpha,
      And (T <= S) (And (Stationary T) (Set.EqOn f (fun _ => beta) T)) := by
  cases exists_stationaryFiber_of_range_subset_small hcof hBsmall hS hSubset with
  | intro beta hFiber =>
      exact Exists.intro beta
        (exists_stationary_subset_eqOn_const_of_stationaryFiber hFiber)

theorem exists_stationary_subset_mem_eqOn_const_of_range_subset_small
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
  ...
  | intro beta hbeta =>
      exact Exists.intro beta
        (And.intro hbeta.left
          (exists_stationary_subset_eqOn_const_of_stationaryFiber hbeta.right))

theorem exists_stationaryFiber_mem_lt_of_range_subset_small
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
    ...
    exists beta,
      And (B beta)
        (And (exists x, And (S x) (And (f x = beta) (beta < x)))
          (Stationary (StationaryFiber S f beta))) := by
  cases exists_stationaryFiber_mem_of_range_subset_small
      hcof hBsmall hS hSubset with
  | intro beta hbeta =>
      cases regressiveOn_stationaryFiber_exists_lt hreg hbeta.right with
      | intro x hx =>
          exact Exists.intro beta
            (And.intro hbeta.left (And.intro (Exists.intro x hx) hbeta.right))

theorem exists_stationaryFiber_lt_of_range_subset_small
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
    ...
    exists beta,
      And (exists x, And (S x) (And (f x = beta) (beta < x)))
        (Stationary (StationaryFiber S f beta)) := by
  cases exists_stationaryFiber_mem_lt_of_range_subset_small
      hcof hBsmall hS hSubset hreg with
  | intro beta hbeta =>
      exact Exists.intro beta hbeta.right

theorem exists_stationary_subset_mem_lt_eqOn_const_of_range_subset_small
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
  ...
  | intro beta hbeta =>
      exact Exists.intro beta
        (And.intro hbeta.left
          (And.intro hbeta.right.left
            (exists_stationary_subset_eqOn_const_of_stationaryFiber
              hbeta.right.right)))

theorem exists_stationaryFiber_of_small_range
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
    ...
    (hRangeSmall :
      Cardinal.lift.{u, u}
          (Cardinal.mk { beta : alpha // exists x, And (S x) (f x = beta) }) <
        Cardinal.lift.{u, u} (Order.cof alpha))
    (hS : Stationary S) :
    exists beta, Stationary (StationaryFiber S f beta) := by
  let idx := { beta : alpha // exists x, And (S x) (f x = beta) }
  let value : idx -> alpha := fun beta => beta.1
  have hCover : forall x, S x -> exists i, f x = value i := by
    intro x hxS
    exact Exists.intro
      (Subtype.mk (f x) (Exists.intro x (And.intro hxS rfl)))
      rfl
  cases exists_stationaryFiber_of_small_value_cover
      (idx := idx)
      hcof
      (by simpa [idx] using hRangeSmall)
      hS
      hCover with
  | intro i hi =>
      exact Exists.intro (value i) hi

theorem exists_stationary_subset_eqOn_const_of_small_range
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
    ...
    exists beta, exists T : Set alpha,
      And (T <= S) (And (Stationary T) (Set.EqOn f (fun _ => beta) T)) := by
  cases exists_stationaryFiber_of_small_range hcof hRangeSmall hS with
  | intro beta hFiber =>
      exact Exists.intro beta
        (exists_stationary_subset_eqOn_const_of_stationaryFiber hFiber)

theorem exists_stationaryFiber_mem_range_of_small_range
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
    ...
    (hS : Stationary S) :
    exists beta,
      And (exists x, And (S x) (f x = beta))
        (Stationary (StationaryFiber S f beta)) := by
  cases exists_stationaryFiber_of_small_range hcof hRangeSmall hS with
  | intro beta hFiber =>
      cases stationaryFiber_exists_mem_eq hFiber with
      | intro x hx =>
          exact Exists.intro beta
            (And.intro (Exists.intro x hx) hFiber)

theorem exists_stationary_subset_mem_range_eqOn_const_of_small_range
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
    ...
    exists beta,
      And (exists x, And (S x) (f x = beta))
        (exists T : Set alpha,
          And (T <= S) (And (Stationary T) (Set.EqOn f (fun _ => beta) T))) := by
  cases exists_stationaryFiber_mem_range_of_small_range hcof hRangeSmall hS with
  | intro beta hbeta =>
      exact Exists.intro beta
        (And.intro hbeta.left
          (exists_stationary_subset_eqOn_const_of_stationaryFiber hbeta.right))

theorem exists_stationaryFiber_mem_range_lt_of_small_range
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
  ...
  cases exists_stationaryFiber_of_small_range hcof hRangeSmall hS with
  | intro beta hFiber =>
      cases regressiveOn_stationaryFiber_exists_lt hreg hFiber with
      | intro x hx =>
          exact Exists.intro beta
            (And.intro (Exists.intro x hx) hFiber)

theorem exists_stationary_subset_mem_range_lt_eqOn_const_of_small_range
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
    ...
    exists beta,
      And (exists x, And (S x) (And (f x = beta) (beta < x)))
        (exists T : Set alpha,
          And (T <= S) (And (Stationary T) (Set.EqOn f (fun _ => beta) T))) := by
  cases exists_stationaryFiber_mem_range_lt_of_small_range hcof hRangeSmall hS hreg with
  | intro beta hbeta =>
      exact Exists.intro beta
        (And.intro hbeta.left
          (exists_stationary_subset_eqOn_const_of_stationaryFiber hbeta.right))

theorem stationary_iUnion_iff_of_small
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    {idx : Type v}
    ...
    (hidx :
      Cardinal.lift.{u, v} (Cardinal.mk idx) <
        Cardinal.lift.{v, u} (Order.cof alpha))
    {S : idx -> Set alpha} :
    Stationary (Set.iUnion S) <-> exists i, Stationary (S i) := by
  constructor
  · intro hUnion
    exact exists_stationary_of_stationary_iUnion_of_small hcof hidx hUnion
  · rintro ⟨i, hSi⟩
    exact stationary_iUnion_of_stationary hSi
...
theorem exists_stationary_eqOn_const_of_small_codomain
  ...
  have hStationaryUnion : Stationary (Set.iUnion fiber) := by
    ...
    exact hS
  obtain ⟨b, hb⟩ :=
    (stationary_iUnion_iff_of_small hcof hSmall).mp hStationaryUnion
...
theorem not_injOn_of_isCofinal_of_mk_lt_cof
    {alpha beta : Type u}
    [Preorder alpha]
    (hSmall : Cardinal.mk beta < Order.cof alpha)
    (f : alpha -> beta)
    {S : Set alpha}
    (hS : IsCofinal S) :
    Not (Set.InjOn f S) := by
  intro hInjective
  have hSubtypeInjective : Function.Injective
      (fun x : S => f x.1) := by
    intro x y hxy
    exact Subtype.ext (hInjective x.2 y.2 hxy)
...
set, values in the corresponding small coordinate type cannot be
injective. -/
theorem exists_stationary_eqOn_const_and_not_injOn_values
    {alpha A : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    [Nonempty alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
    (hASmall : Cardinal.mk A < Order.cof alpha)
    (coordinate : alpha -> A)
    (Value : A -> Type u)
    (values : forall a, alpha -> Value a)
    (hValueSmall : forall a, Cardinal.mk (Value a) < Order.cof alpha) :
    exists a : A, exists T : Set alpha,
      Stationary T /\
        Set.EqOn coordinate (fun _ => a) T /\
          Not (Set.InjOn (values a) T) := by
  obtain ⟨a, T, _hTSub, hTStationary, hCoordinate⟩ :=
    exists_stationary_eqOn_const_of_small_codomain
      hcof (by simpa using hASmall) coordinate (univ_stationary hcof)
...
strictly increases whenever a coordinate is repeated.  This is the terminal
cardinal contradiction in the proof of Lemma 24.14. -/
theorem no_repeated_coordinate_strict_pattern_of_small
    {alpha A beta : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    [Nonempty alpha]
    ...
    (hValueSmall : forall a,
      Cardinal.mk (Set.range (values a)) < Order.cof alpha)
    (hStrict : forall {i j}, i < j -> coordinate i = coordinate j ->
  ...
  have hInjective : Set.InjOn valueInRange T := by
    ...
    · by_contra hijOrder
      ...
      have hValueLt := hStrict hjiLt hCoordEq
    ...
    · by_contra hjiOrder
      ...
      have hValueLt := hStrict hijLt hCoordEq
      have hValueEq : values a i = values a j := congrArg Subtype.val hij
      rw [hCoordinate hi, hCoordinate hj, hValueEq] at hValueLt
      exact (lt_irrefl _ hValueLt)
  exact not_injOn_of_isCofinal_of_mk_lt_cof
...
theorem no_repeated_coordinate_strict_pattern_of_lift_small
  ...
  obtain ⟨a, T, _hTSub, hTStationary, hCoordinate⟩ :=
    exists_stationary_eqOn_const_of_small_codomain
      hcof hASmall coordinate (univ_stationary hcof)
  let valueInRange : alpha -> Set.range (values a) := fun i =>
    ⟨values a i, Set.mem_range_self i⟩
  have hInjective : Set.InjOn valueInRange T := by
    intro i hi j hj hij
...
theorem exists_stationaryFiber_of_diagonalCofinalHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hCofinal : DiagonalCofinalHypothesis (alpha := alpha))
    {S : Set alpha}
    ...
    (hS : Stationary S)
    (hreg : RegressiveOn S f) :
    exists beta, Stationary (StationaryFiber S f beta) :=
  exists_stationaryFiber_of_diagonalClubHypothesis
    (diagonalClubHypothesis_of_diagonalCofinalHypothesis hCofinal)
    hS
    hreg

theorem exists_stationaryFiber_lt_of_diagonalClubHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hDiag : DiagonalClubHypothesis (alpha := alpha))
    {S : Set alpha}
    ...
    exists beta,
      And (exists x, And (S x) (And (f x = beta) (beta < x)))
        (Stationary (StationaryFiber S f beta)) := by
  cases exists_stationaryFiber_of_diagonalClubHypothesis hDiag hS hreg with
  | intro beta hFiber =>
      cases regressiveOn_stationaryFiber_exists_lt hreg hFiber with
      | intro x hx =>
          exact Exists.intro beta
...
theorem exists_stationaryFiber_mem_range_of_diagonalClubHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hDiag : DiagonalClubHypothesis (alpha := alpha))
    {S : Set alpha}
    ...
    exists beta,
      And (exists x, And (S x) (f x = beta))
        (Stationary (StationaryFiber S f beta)) := by
  cases exists_stationaryFiber_lt_of_diagonalClubHypothesis hDiag hS hreg with
  | intro beta hbeta =>
      cases hbeta.left with
      | intro x hx =>
          exact Exists.intro beta
            (And.intro (Exists.intro x (And.intro hx.left hx.right.left))
              hbeta.right)

theorem exists_stationaryFiber_mem_of_range_subset_diagonalClubHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hDiag : DiagonalClubHypothesis (alpha := alpha))
    {B S : Set alpha}
    ...
    (hreg : RegressiveOn S f) :
    exists beta, And (B beta) (Stationary (StationaryFiber S f beta)) := by
  cases exists_stationaryFiber_mem_range_of_diagonalClubHypothesis hDiag hS hreg with
  | intro beta hbeta =>
      cases hbeta.left with
      | intro x hx =>
          have hB : B (f x) := hSubset x hx.left
          rw [hx.right] at hB
          exact Exists.intro beta (And.intro hB hbeta.right)

theorem exists_stationaryFiber_mem_lt_of_range_subset_diagonalClubHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hDiag : DiagonalClubHypothesis (alpha := alpha))
    {B S : Set alpha}
  ...
  | intro beta hbeta =>
      ...
      | intro x hx =>
          have hB : B (f x) := hSubset x hx.left
          rw [hx.right.left] at hB
          exact Exists.intro beta
            (And.intro hB (And.intro (Exists.intro x hx) hbeta.right))

theorem exists_stationary_subset_eqOn_const_of_diagonalClubHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hDiag : DiagonalClubHypothesis (alpha := alpha))
    {S : Set alpha}
...
theorem exists_stationary_subset_lt_eqOn_const_of_diagonalClubHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hDiag : DiagonalClubHypothesis (alpha := alpha))
    {S : Set alpha}
...
theorem exists_stationary_subset_mem_range_eqOn_const_of_diagonalClubHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hDiag : DiagonalClubHypothesis (alpha := alpha))
    {S : Set alpha}
...
theorem exists_stationary_subset_mem_eqOn_const_of_range_subset_diagonalClubHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hDiag : DiagonalClubHypothesis (alpha := alpha))
    {B S : Set alpha}
    ...
    exists beta,
      And (B beta)
        (exists T : Set alpha,
          And (T <= S) (And (Stationary T) (Set.EqOn f (fun _ => beta) T))) := by
  cases exists_stationaryFiber_mem_of_range_subset_diagonalClubHypothesis
      hDiag hS hSubset hreg with
  | intro beta hbeta =>
      exact Exists.intro beta
        (And.intro hbeta.left
          (exists_stationary_subset_eqOn_const_of_stationaryFiber hbeta.right))

theorem exists_stationary_subset_mem_lt_eqOn_const_of_range_subset_diagonalClubHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hDiag : DiagonalClubHypothesis (alpha := alpha))
    {B S : Set alpha}
    ...
    exists beta,
      And (B beta)
        (And (exists x, And (S x) (And (f x = beta) (beta < x)))
          (exists T : Set alpha,
            And (T <= S) (And (Stationary T) (Set.EqOn f (fun _ => beta) T)))) := by
  cases exists_stationaryFiber_mem_lt_of_range_subset_diagonalClubHypothesis
      hDiag hS hSubset hreg with
  | intro beta hbeta =>
      exact Exists.intro beta
        (And.intro hbeta.left
...
theorem exists_stationaryFiber_lt_of_diagonalCofinalHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hCofinal : DiagonalCofinalHypothesis (alpha := alpha))
    {S : Set alpha}
    ...
    exists beta,
      And (exists x, And (S x) (And (f x = beta) (beta < x)))
        (Stationary (StationaryFiber S f beta)) :=
  exists_stationaryFiber_lt_of_diagonalClubHypothesis
    (diagonalClubHypothesis_of_diagonalCofinalHypothesis hCofinal)
    hS
    hreg

theorem exists_stationaryFiber_mem_range_of_diagonalCofinalHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hCofinal : DiagonalCofinalHypothesis (alpha := alpha))
    {S : Set alpha}
...
theorem exists_stationaryFiber_mem_of_range_subset_diagonalCofinalHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hCofinal : DiagonalCofinalHypothesis (alpha := alpha))
    {B S : Set alpha}
    ...
    (hSubset : forall x, S x -> B (f x))
    (hreg : RegressiveOn S f) :
    exists beta, And (B beta) (Stationary (StationaryFiber S f beta)) :=
  exists_stationaryFiber_mem_of_range_subset_diagonalClubHypothesis
    (diagonalClubHypothesis_of_diagonalCofinalHypothesis hCofinal)
    hS hSubset hreg

theorem exists_stationaryFiber_mem_lt_of_range_subset_diagonalCofinalHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hCofinal : DiagonalCofinalHypothesis (alpha := alpha))
    {B S : Set alpha}
...
theorem exists_stationary_subset_eqOn_const_of_diagonalCofinalHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hCofinal : DiagonalCofinalHypothesis (alpha := alpha))
    {S : Set alpha}
    ...
    (hreg : RegressiveOn S f) :
    exists beta, exists T : Set alpha,
      And (T <= S) (And (Stationary T) (Set.EqOn f (fun _ => beta) T)) :=
  exists_stationary_subset_eqOn_const_of_diagonalClubHypothesis
    (diagonalClubHypothesis_of_diagonalCofinalHypothesis hCofinal)
    hS
    hreg

theorem exists_stationary_subset_lt_eqOn_const_of_diagonalCofinalHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hCofinal : DiagonalCofinalHypothesis (alpha := alpha))
    {S : Set alpha}
...
theorem exists_stationary_subset_mem_range_eqOn_const_of_diagonalCofinalHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hCofinal : DiagonalCofinalHypothesis (alpha := alpha))
    {S : Set alpha}
...
theorem exists_stationary_subset_mem_eqOn_const_of_range_subset_diagonalCofinalHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hCofinal : DiagonalCofinalHypothesis (alpha := alpha))
    {B S : Set alpha}
...
theorem exists_stationary_subset_mem_lt_eqOn_const_of_range_subset_diagonalCofinalHypothesis
    {alpha : Type u}
    [LinearOrder alpha]
    (hCofinal : DiagonalCofinalHypothesis (alpha := alpha))
    {B S : Set alpha}
...
theorem exists_stationaryFiber_of_small_initial_segments
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
...
theorem exists_stationary_subset_mem_range_lt_eqOn_const_of_small_initial_segments
    {alpha : Type u}
    [LinearOrder alpha]
    [WellFoundedLT alpha]
    (hcof : Not (Order.cof alpha = Cardinal.aleph0))
...
theorem exists_stationaryFiber_ord_toType_of_isRegular_of_aleph0_lt
    {c : Cardinal.{u}}
    (hc : c.IsRegular)
    (huncountable : Cardinal.aleph0 < c)
    {S : Set c.ord.ToType}
    {f : c.ord.ToType -> c.ord.ToType}
    (hS : Stationary S)
    (hreg : RegressiveOn S f) :
    exists beta, Stationary (StationaryFiber S f beta) :=
  exists_stationaryFiber_of_small_initial_segments
    (cof_ord_toType_ne_aleph0_of_isRegular_of_aleph0_lt hc huncountable)
    (smallInitialSegments_ord_toType_of_isRegular hc)
    hS
    hreg

theorem exists_stationary_subset_eqOn_const_ord_toType_of_isRegular_of_aleph0_lt
    {c : Cardinal.{u}}
    (hc : c.IsRegular)
    (huncountable : Cardinal.aleph0 < c)
    {S : Set c.ord.ToType}
    ...
    (hS : Stationary S)
    (hreg : RegressiveOn S f) :
    exists beta, exists T : Set c.ord.ToType,
      And (T <= S) (And (Stationary T) (Set.EqOn f (fun _ => beta) T)) :=
  exists_stationary_subset_eqOn_const_of_small_initial_segments
    (cof_ord_toType_ne_aleph0_of_isRegular_of_aleph0_lt hc huncountable)
    (smallInitialSegments_ord_toType_of_isRegular hc)
...
theorem exists_stationary_subset_mem_range_lt_eqOn_const_ord_toType_of_isRegular_of_aleph0_lt
    {c : Cardinal.{u}}
    (hc : c.IsRegular)
    (huncountable : Cardinal.aleph0 < c)
    {S : Set c.ord.ToType}
...
theorem exists_omegaOneCofinalSubset_of_club
    ...
    exists X : Set (Ordinal.{u}),
      X ⊆ (fun i : Set.Iio delta => i.1) '' C /\
        Ordinal.type ((· < ·) : X -> X -> Prop) =
          Ordinal.omega.{u + 1} 1 /\
        sSup X = delta := by
  let inclusion : C -> Set.Iio delta := fun x => x.1
  have hInclusionStrict : StrictMono inclusion := by
    intro x y hxy
    exact hxy
  have hInclusionCofinal : IsCofinal (Set.range inclusion) := by
    simpa only [inclusion, Subtype.range_coe_subtype] using hC.isCofinal
  have hCofC : Order.cof C = Cardinal.aleph (1 : Ordinal.{u + 1}) := by
    rw [Order.cof_congr_of_strictMono hInclusionStrict hInclusionCofinal,
      Ordinal.cof_Iio, ← Ordinal.lift_cof, hDeltaCof,
      Cardinal.lift_aleph]
    simp only [Ordinal.lift_one]
  obtain ⟨S, hSCofinal, hSType⟩ := Ordinal.exists_ord_cof_eq C
  let value : C -> Ordinal.{u} := fun x => x.1.1
  let X : Set (Ordinal.{u}) := value '' S
  have hValueStrict : StrictMono value := by
    intro x y hxy
    exact hxy
  have hTypeX : Ordinal.type ((· < ·) : X -> X -> Prop) =
    ...
    have hIso : S ≃o X :=
      StrictMonoOn.orderIso value S (hValueStrict.strictMonoOn S)
    rw [← hIso.toRelIsoLT.ordinal_type_eq, hSType, hCofC,
      Cardinal.ord_aleph]
  have hXSubset : X ⊆ (fun i : Set.Iio delta => i.1) '' C := by
    rintro x ⟨c, _hcS, rfl⟩
    exact ⟨c.1, c.2, rfl⟩
  have hSNonempty : S.Nonempty := by
    haveI : Nonempty C := by
      haveI : Nonempty (Set.Iio delta) := by
        rw [← Order.cof_ne_zero_iff]
        rw [← Order.cof_congr_of_strictMono hInclusionStrict
          hInclusionCofinal, hCofC]
        exact ne_of_gt ((Cardinal.aleph0_pos).trans_le
          (Cardinal.aleph0_le_aleph 1))
      obtain ⟨c, hcC, _hc⟩ :=
        hC.isCofinal (Classical.choice inferInstance)
      exact ⟨⟨c, hcC⟩⟩
    obtain ⟨s, hsS, _⟩ :=
      hSCofinal (Classical.choice inferInstance)
    exact ⟨s, hsS⟩
  have hXNonempty : X.Nonempty := hSNonempty.image value
  have hXBdd : BddAbove X := by
    refine ⟨delta, ?_⟩
    rintro x ⟨c, _hcS, rfl⟩
    exact c.1.2.le
  have hSupLe : sSup X <= delta :=
    csSup_le hXNonempty (fun x hx => by
      obtain ⟨i, _hiC, rfl⟩ := hXSubset hx
      exact i.2.le)
  have hDeltaLe : delta <= sSup X := by
    ...
    intro beta hBeta
    obtain ⟨d, hdC, hBetaD⟩ :=
      hC.isCofinal ⟨beta + 1, hDeltaLimit.succ_lt hBeta⟩
    obtain ⟨c, hcS, hDC⟩ := hSCofinal ⟨d, hdC⟩
    have hValueMem : value c ∈ X := ⟨c, hcS, rfl⟩
    have hBetaValue : beta < value c := by
      exact (lt_add_one beta).trans_le (hBetaD.trans hDC)
...
later stage demanded by a counterexample at the current stage.  The recursive
sequence stays below `lambda` and every later value lies beyond all earlier
requested advances. -/
theorem exists_strictMono_advance_sequence
    {gamma lambda : Cardinal.{u}}
    (hLambda : lambda.IsRegular)
    (hGammaLambda : gamma < lambda)
  ...
  have hStep : forall (alpha : I)
      (previous : forall beta, beta < alpha -> Set.Iio lambda.ord),
      exists q : Set.Iio lambda.ord,
        forall beta hbeta, advance (previous beta hbeta) < q := by
    intro alpha previous
    let candidate : Ordinal.{u} :=
      iSup fun beta : I => if hbeta : beta < alpha then
        (advance (previous beta hbeta)).1 + 1 else 0
    have hCandidate : candidate < lambda.ord := by
      apply Ordinal.iSup_lt_of_lt_cof
    ...
    intro beta hbeta
    change (advance (previous beta hbeta)).1 < candidate
  ...
  have hStageEq : forall alpha,
      stage alpha = Classical.choose
        (hStep alpha (fun beta hbeta => stage beta)) := by
    intro alpha
    exact WellFounded.fix_eq _ _ alpha
  have hAdvanceStage : forall i j, i < j -> advance (stage i) < stage j := by
    intro i j hij
    rw [hStageEq j]
    exact Classical.choose_spec
      (hStep j (fun beta hbeta => stage beta)) i hij
