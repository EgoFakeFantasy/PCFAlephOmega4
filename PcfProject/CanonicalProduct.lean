...
family: every earlier family member lies below the later envelope.  If the
coordinate index and every coordinate cardinal are smaller than the regular
length `gamma`, the failure recursion above would produce a repeated
coordinate whose intermediate values strictly increase on a cofinal set,
contradicting coordinate smallness. -/
theorem exists_eventualDominatingStage_of_rapidComparison_of_small
    {A : CardSet.{u}}
    {J : Ideal (CardinalIndex A)}
    {gamma lambda : Cardinal.{u}}
    (hGamma : gamma.IsRegular)
    ...
    (hCoordinateSmall : forall k : CardinalIndex A, k.1 < gamma)
    (f s : Set.Iio lambda.ord ->
      ProductElement (cardinalProductFrame A J))
    (hStart : forall alpha,
      (cardinalProductFrame A J).eventuallyPointwiseLt
        (s alpha) (f alpha))
    (hRapidComparison : forall {alpha beta : Set.Iio lambda.ord},
      alpha < beta -> forall k : CardinalIndex A,
        cardinalProductOrdinalValue (f alpha) k <=
          cardinalProductOrdinalValue (s beta) k) :
  ...
  obtain ⟨stage, next, h, coordinate, _hStageStrict, _hStageNext,
    hNextBeforeLater, hChain⟩ :=
    exists_cardinalProduct_failureStagePattern hLambda hGammaLambda
      f s hStart hNoStage
  haveI : Nonempty gamma.ord.ToType :=
    ⟨Ordinal.ToType.mk ⟨0, hGamma.ord_pos⟩⟩
  have hCofEq : Order.cof gamma.ord.ToType = gamma := by
    rw [Ordinal.cof_toType, hGamma.cof_ord]
  have hCofNe : Not (Order.cof gamma.ord.ToType = Cardinal.aleph0) := by
    rw [hCofEq]
    exact ne_of_gt hGammaUncountable
  let values : CardinalIndex A -> gamma.ord.ToType -> Ordinal.{u} :=
    fun k i => cardinalProductOrdinalValue (h i) k
  have hValueSmall : forall k,
      Cardinal.lift.{u, u + 1} (Cardinal.mk (Set.range (values k))) <
        Cardinal.lift.{u + 1, u} (Order.cof gamma.ord.ToType) := by
    intro k
    rw [hCofEq]
    let embed : Set.range (values k) -> Set.Iio k.1.ord := fun x =>
      ⟨x.1, by
        obtain ⟨i, hi⟩ := x.2
        rw [← hi]
        exact cardinalProductOrdinalValue_lt (h i) k⟩
    have hEmbed : Function.Injective embed := by
      intro x y hxy
      apply Subtype.ext
      exact congrArg (fun z : Set.Iio k.1.ord => z.1) hxy
    have hRangeSmall : Cardinal.mk (Set.range (values k)) <
        Cardinal.lift.{u + 1} gamma := calc
      Cardinal.mk (Set.range (values k)) <=
          Cardinal.mk (Set.Iio k.1.ord) :=
        Cardinal.mk_le_of_injective hEmbed
      _ = Cardinal.lift.{u + 1} k.1 := by
        rw [Cardinal.mk_Iio_ordinal, Cardinal.card_ord]
      _ < Cardinal.lift.{u + 1} gamma :=
        Cardinal.lift_lt.mpr (hCoordinateSmall k)
    rw [Cardinal.lift_id'.{u, u + 1}]
    exact hRangeSmall
  apply no_repeated_coordinate_strict_pattern_of_lift_small hCofNe
    (by simpa only [hCofEq] using hIndexSmall)
      coordinate values hValueSmall
  intro i j hij hCoordinateEq
  have hFirst := (hChain i).2.2
  have hMiddle := hRapidComparison (hNextBeforeLater i j hij) (coordinate i)
  have hLast := (hChain j).1.trans (hChain j).2.1
  dsimp only [values]
  have hMiddle' : cardinalProductOrdinalValue (f (next i)) (coordinate i) <=
...
belong pointwise to its product.  Thus the contradiction uses `|S_a| <
gamma`, not the stronger and generally false requirement that the whole
coordinate cardinal be below `gamma`. -/
theorem exists_eventualDominatingStage_of_rapidComparison_of_smallSets
    {A : CardSet.{u}}
    {J : Ideal (CardinalIndex A)}
    {gamma lambda : Cardinal.{u}}
    (hGamma : gamma.IsRegular)
  ...
  obtain ⟨stage, _hStageStrict, hNextBeforeLater⟩ :=
    exists_strictMono_advance_sequence hLambda hGammaLambda next hStageNext
  let h : gamma.ord.ToType -> ProductElement (cardinalProductFrame A J) :=
    fun i => counter (stage i)
  have hChain : forall i, exists k : CardinalIndex A,
  ...
  have hValueSmall : forall k,
    ...
    let embed : Set.range (values k) -> S k := fun x =>
      ⟨x.1, by
        obtain ⟨i, hi⟩ := x.2
        rw [← hi]
        exact hCounterAllowed (stage i) k⟩
    have hEmbed : Function.Injective embed := by
      intro x y hxy
      apply Subtype.ext
  ...
  apply no_repeated_coordinate_strict_pattern_of_lift_small hCofNe
    (by simpa only [hCofEq] using hIndexSmall)
      coordinate values hValueSmall
  intro i j hij hCoordinateEq
  have hFirst := (Classical.choose_spec (hChain i)).2.2
  have hMiddle := hRapidComparison
    (hNextBeforeLater i j hij) (coordinate i)
  have hLast := (Classical.choose_spec (hChain j)).1.trans
    (Classical.choose_spec (hChain j)).2.1
  dsimp only [values]
  have hMiddle' :
      cardinalProductOrdinalValue (f (next (stage i))) (coordinate i) <=
        cardinalProductOrdinalValue (s (stage j)) (coordinate j) :=
    hMiddle.trans_eq (by rw [hCoordinateEq])
  exact (hFirst.trans_le hMiddle').trans hLast
...
theorem exists_eventualDominatingStage_of_closedRapidComparison_of_smallSets
    {A : CardSet.{u}}
    {J : Ideal (CardinalIndex A)}
    {gamma lambda : Cardinal.{u}}
    (hGamma : gamma.IsRegular)
    ...
    (f : Set.Iio lambda.ord ->
      ProductElement (cardinalProductFrame A J))
    (s : Set.Iio lambda.ord -> CardinalProductClosedElement A)
    (hStart : forall alpha,
      CardinalProductClosedEventuallyLtProduct J (s alpha) (f alpha))
    (hRapidComparison : forall {alpha beta : Set.Iio lambda.ord},
      alpha < beta -> forall k : CardinalIndex A,
        cardinalProductOrdinalValue (f alpha) k <= (s beta k).1) :
    exists alpha : Set.Iio lambda.ord,
      forall h : ProductElement (cardinalProductFrame A J),
        (forall k, cardinalProductOrdinalValue h k ∈ S k) ->
        (cardinalProductFrame A J).eventuallyPointwiseLt (f alpha) h ->
  ...
  let h : gamma.ord.ToType -> ProductElement (cardinalProductFrame A J) :=
    fun i => counter (stage i)
  have hChain : forall i, exists k : CardinalIndex A,
      (s (stage i) k).1 < cardinalProductOrdinalValue (f (stage i)) k /\
        cardinalProductOrdinalValue (f (stage i)) k <
          cardinalProductOrdinalValue (h i) k /\
        cardinalProductOrdinalValue (h i) k <
          cardinalProductOrdinalValue (f (next (stage i))) k := by
    ...
    exists alpha : Set.Iio lambda.ord,
      forall h : ProductElement (cardinalProductFrame A J),
        ...
        (cardinalProductFrame A J).eventuallyPointwiseLt (f alpha) h ->
          forall next : Set.Iio lambda.ord, alpha < next ->
            (cardinalProductFrame A J).eventuallyLe (f next) h := by
  apply exists_eventualDominatingStage_of_gammaRapid_of_smallSets_of_support
    hGamma hGammaUncountable hLambda hGammaLambda
    (fun _ => True)
...
theorem cardinalProductQuotient_lift_lt_cof_of_regulars_of_eventually
    {A : CardSet.{u}}
    (hRegulars : SetOfRegulars A)
    {kappa : Cardinal.{u}}
    {B : CardinalIndex A -> Prop}
    (hCoordinate : forall i, B i -> kappa < i.1)
    {J : Ideal (CardinalIndex A)}
    (hProper : J.IsProper)
    (hEventually : J.Eventually B) :
    Cardinal.lift.{u + 1} kappa <
...
necessary universe lift). This remains a lower bound on an arbitrary
cofinal set; it does not turn that set into a quotient scale. -/
theorem cardinalProductQuotient_lift_le_cof_of_regulars_of_eventually_unbounded
    {A : CardSet.{u}}
    (hRegulars : SetOfRegulars A)
    {lambda : Cardinal.{u}}
    {J : Ideal (CardinalIndex A)}
    (hCoordinate : forall kappa, kappa < lambda ->
      exists B : CardinalIndex A -> Prop,
        J.Eventually B /\ forall i, B i -> kappa < i.1)
    (hProper : J.IsProper) :
    Cardinal.lift.{u + 1} lambda <=
      Order.cof (CardinalProductQuotient A J) := by
  classical
  by_contra hNot
  have hCofLt : Order.cof (CardinalProductQuotient A J) <
      Cardinal.lift.{u + 1} lambda :=
    lt_of_not_ge hNot
  obtain ⟨s, hSetCofinal, hCard⟩ :=
    Order.exists_cof_eq (CardinalProductQuotient A J)
  have hSmall : Cardinal.mk s < Cardinal.lift.{u + 1} lambda := by
    rw [hCard]
    exact hCofLt
  have hEmbed : Nonempty (s ↪ ULift.{u + 1} lambda.out) := by
    apply (Cardinal.le_def _ _).mp
    simpa only [Cardinal.mk_uLift, Cardinal.mk_out] using hSmall.le
  letI : Small.{u} s :=
    small_of_injective (Classical.choice hEmbed).injective
  let dQuotient : Shrink.{u} s -> CardinalProductQuotient A J :=
    fun i => ((equivShrink s).symm i).1
  ...
  obtain ⟨d, hRawCofinal⟩ :=
    cardinalProductFrame_exists_cofinalFamily_of_quotient
      dQuotient hQuotientCofinal
  have hShrinkLt : Cardinal.mk (Shrink.{u} s) < lambda := by
    ...
    calc
      Cardinal.lift.{u + 1} (Cardinal.mk (Shrink.{u} s)) =
        ...
        Cardinal.lift_mk_shrink' s
      _ = Cardinal.mk s := Cardinal.lift_id'.{u, u + 1} _
      _ < Cardinal.lift.{u + 1} lambda := hSmall
  obtain ⟨B, hEventually, hCoordinateAt⟩ :=
    hCoordinate (Cardinal.mk (Shrink.{u} s)) hShrinkLt
  exact (cardinalProductFrame_not_isCofinalFamily_of_mk_lt_of_eventually
    hRegulars
...
theorem CardinalProductClosedExactUpperBound.cardinal_le_of_cofinalBelow
    {A : CardSet.{u}}
    {J : Ideal (CardinalIndex A)}
    {theta : Cardinal.{u}}
    (hThetaRegular : Cardinal.IsRegular theta)
    ...
    (hCofinalBelow : forall h : ProductElement (cardinalProductFrame A J),
      CardinalProductEventuallyLtClosed J h f ->
        exists i, (cardinalProductFrame A J).eventuallyLe h (e i)) :
    theta <= Cardinal.mk iota := by
  classical
  by_contra hNotLe
  have hShort : Cardinal.mk iota < theta := lt_of_not_ge hNotLe
  let stage : iota -> (cardinalScaleLength theta).Level := fun i =>
  ...
  obtain ⟨beta, hStageBeta⟩ :=
    exists_strict_upper_bound_of_mk_lt_regular
      hThetaRegular hShort stage
  obtain ⟨i, hBetaLe⟩ := hCofinalBelow (d beta) (hExact.1 beta)
  have hStrict :=
    (cardinalProductFrame A J).eventuallyLt_of_eventually_pointwiseStrict
      hProper (hIncreasing (by
        simpa only [cardinalScaleLength] using hStageBeta i))
  exact hStrict.right
